...
def get_ffmpeg_cmd_pci_stess_test(out_file:str, in_n: int):
...
def get_ffmpeg_cmd_cpu(out_file:str, in_n: int):
    base = ["time","-f", "%e", "--output", out_file, 
              "ffmpeg", "-loglevel", "error", "-hide_banner"]

    for _ in range(in_n):
        base += ["-i", "/dev/shm/cut.mp4", "-t", "60", "-f", "null", "-"] 

    return base

def get_ffmpeg_cmd_gpu(out_file:str, in_n: int):
    base = ["time","-f", "%e", "--output", out_file, 
              "ffmpeg", "-loglevel", "error", "-hide_banner"]

    for _ in range(in_n):
        base += ["-c:v", "h264_cuvid", "-i", "/dev/shm/cut.mp4", "-t", "60", "-f", "null", "-"] 
...
def generate_pci_graph():
    ...
    for fold in range(1, FOLDS):
        ...
        for pci_gen in pci_gens:
            ...
            _sum = list(map(lambda _x: _x[0] + _x[1], zip(rxs[0], txs[0])))
        ...
        plt.savefig(os.path.join(DIR_NAME, f"pci_seconds_graph_{prefix}.png"))
        plt.clf()
...
def main():
    ...
    for i in range(1, FOLDS):
        afterfix = f"_{type_run}_{pci_gen}_{pci_width}_{i}"
        REPORT_OUT_FILE = os.path.join(DIR_NAME, f"report{afterfix}.txt")
        GPU_OUT_FILE = get_gpu_stats_path(afterfix)
        FFMPEG_CMD = get_ffmpeg_cmd_pci_stess_test(REPORT_OUT_FILE, i) if type_run == "stress_test" else get_ffmpeg_cmd_gpu(REPORT_OUT_FILE, i) if type_run == "gpu" else get_ffmpeg_cmd_cpu(REPORT_OUT_FILE, i)
        ffmpeg_out = open(REPORT_OUT_FILE, "a+")
        errors_out = open(STD_ERR, "a+")
